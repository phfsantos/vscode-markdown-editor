...
function testStringBasedLineTracking() {
  ...
  function getLineTextAtOffset(content, offset) {
  ...
  const wikiOffset = content.indexOf('[[Target]]');
  const markdownOffset = content.indexOf('[link](./Target.md)');

  assert.strictEqual(getLineNumberAtOffset(content, wikiOffset), 1);
  assert.strictEqual(getLineTextAtOffset(content, wikiOffset), 'See [[Target]] here');
  assert.strictEqual(getLineNumberAtOffset(content, markdownOffset), 2);
  assert.strictEqual(getLineTextAtOffset(content, markdownOffset), 'Another [link](./Target.md)');
