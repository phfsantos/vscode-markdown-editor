...
export class RelationshipAnalyzer {
  ...
  public async getOutgoingLinks(fileUri: vscode.Uri): Promise<Link[]> {
    ...
    while ((match = mdLinkRegex.exec(content)) !== null) {
      ...
      const lineNumber = this.getLineNumberAtOffset(content, match.index);
  ...
  public async getBacklinks(fileUri: vscode.Uri): Promise<Backlink[]> {
    ...
    for (const file of files) {
      ...
      while ((match = mdLinkRegex.exec(content)) !== null) {
        const lineNumber = this.getLineNumberAtOffset(content, match.index);
        foundLinks.set(lineNumber, this.getLineTextAtOffset(content, match.index).trim());
      }
  ...
  /**
   * Read markdown content without opening a new VS Code document.
   */
  ...
  /**
   * Derive a zero-based line number from a text offset.
   */
  ...
  /**
   * Read the full line containing a text offset.
   */
  private getLineTextAtOffset(content: string, offset: number): string {
    const lineStart = content.lastIndexOf('\n', offset - 1) + 1;
    const rawLineEnd = content.indexOf('\n', offset);
    const lineEnd = rawLineEnd === -1 ? content.length : rawLineEnd;
    return content.slice(lineStart, lineEnd).replace(/\r$/, '');
  }
  ...
  public async getRelatedFiles(fileUri: vscode.Uri, limit: number = 10): Promise<RelatedFile[]> {
    ...
    const currentContent = await this.readFileContent(fileUri);
    ...
    for (const file of allFiles) {
      ...
      const contentSimilarity = await this.calculateContentSimilarity(currentContent, file);
   ...
   */
  private async calculateContentSimilarity(content1: string, file2: vscode.Uri): Promise<number> {
