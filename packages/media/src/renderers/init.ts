...
import { WidgetRenderer } from './builtin/WidgetRenderer';
import { DashboardRenderer } from './builtin/DashboardRenderer';
...
export function initializeRendererSystem(): void {
  ...
  registry.register(widgetRenderer, 'builtin');
  
  const dashboardRenderer = new DashboardRenderer();
  registry.register(dashboardRenderer, 'builtin');
}
    ...
    customRenders.push({
      ...
      render: (code: HTMLElement) => {
        return new Promise(async (resolve) => {
          try {
            // CRITICAL: Skip rendering in the marker (hidden source) area
            // In IR mode, Vditor has both a hidden marker and visible preview
            // We only want to render in the preview area
            const isInMarker = code.closest('.vditor-ir__marker--pre');
            if (isInMarker) {
              resolve(true);
              return;
            }
            ...
            const existingContainer = code.querySelector(`.interactive-table-container, .kanban-board-container, .playground-container, .widget-container, .dashboard-container`);
            ...
            if (code.hasAttribute(`data-${renderer.language}-pending`) || code.hasAttribute(`data-${renderer.language}-rendered`)) {
              resolve(true);
              return;
            }
            ...
            if (code.classList.contains('interactive-table-container') || 
                ...
                code.classList.contains('widget-container') ||
                code.classList.contains('dashboard-container')) {
              resolve(true);
              return;
            }
            ...
            // Vditor may have detached it during re-rendering
            ...
            // Use data-render-pending to indicate init.ts is processing
            // WidgetRenderer will set data-widget-rendered when actually rendered
            code.setAttribute(`data-${renderer.language}-pending`, 'true');
